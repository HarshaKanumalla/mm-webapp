...
export const Chatbot = () => {
  ...
  return (
    <div className="flex h-screen bg-[#F3F4F4] overflow-hidden">
      ...
      {/* Main Content */}
      <div className="flex-1 p-6 pb-4 overflow-hidden">
        {/* Table Container */}
        <div className="bg-[#FFFFFF] rounded-[25px] shadow-md p-6 h-[66.5vh]">
          {/* Table Header */}
          ...
          {/* Table Content */}
          <div className="overflow-y-auto h-[calc(66.5vh-100px)]">
            {isLoading ? (
              ...
              lostReports.map((report) => (
                <div key={report.id} className="flex items-center px-4 py-3 border-b border-gray-200">
                  ...
                  <div className="w-[30%] flex items-center pl-4">
                    {report.image && report.image !== '-' ? (
                      <>
                        ...
                        />
                        <div className="ml-3">
                          <div className="text-sm text-[#858080]">{formatTimestamp(report.timestamp)}</div>
                          <div>{report.description}</div>
                          <div className="text-sm text-[#858080]">{report.location}</div>
                        </div>
                      </>
                    ) : (
                      <div className="ml-3">
                  ...
                  <div className="w-[10%] flex justify-center">
                    <span className="px-4 py-2 rounded-full text-white text-xs bg-[#A14342]">
                      {report.status}
                    </span>
                  </div>
                </div>
      ...
      {selectedImage && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50" onClick={() => setSelectedImage(null)}>
          <div className="relative bg-white p-2 rounded-lg" onClick={e => e.stopPropagation()}>
            <img src={selectedImage} alt="Enlarged view" className="max-h-[80vh] max-w-[80vw] object-contain" />
            <button 
